...
namespace CapShop.CatalogService.Application.Services
{
    public class ProductAppService : IProductAppService
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(dto.Name)) throw new ValidationException("Product name is required.");
            if (dto.Price <= 0) throw new ValidationException("Price must be greater than 0.");
